...
/**
 * An array containing all {@link RandomAccessFile} objects to which the object will write data
 */
	
	protected RandomAccessFile [] raf_list = null;
	
/**
 * An array containing all {@link FileInputStream} objects to which the object will write data
 */
	
	protected FileInputStream [] fis_add = null;
	
/**
 * An array containing a mapped byte buffer for complex reading (and perhaps later writing operation)
 */
	protected MappedByteBuffer [] mapped_list = null;
...
/**
 * 
 * Gets a byte from a specified position in a file
 * 
 ...
 */
	protected byte getByteFrom( short mbbIndex, String order, long offset ) {
		...
		return mapped_list[ mbbIndex ].get( (int) offset );
	}
 ...
 */
	...
	protected long getFileLength( int fileIndex ) throws IOException {
		
		return raf_list != null ? raf_list[fileIndex].length() : mapped_list[fileIndex].capacity();
	}
/**
 ...
 */
	...
	public void close() {
		...
		if( fis_add != null ) {
			...
			for( FileInputStream fis : fis_add ) {
